...
| `update_todo(todo_id, content)` | 내용 수정 | `PATCH /todos/{id}` |
...
```bash
cd /Users/cwjang/workspace/todo_mcp
python3 -m venv .venv
...
1. **FastAPI 앱을 먼저 띄운다** (MCP 서버가 호출할 대상):
   ```bash
   cd /Users/cwjang/workspace/todo_app && ./run.sh   # http://localhost:8000
   ```
2. **Claude Code 에 MCP 서버 등록** (1회만):
   ```bash
   claude mcp add todo -- /Users/cwjang/workspace/todo_mcp/.venv/bin/python /Users/cwjang/workspace/todo_mcp/server.py
   ```
